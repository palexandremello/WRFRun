...
class plotGrid(object):
    ...
    def get_proj_lcc(wps_file):
        """ Essa parte tem que ser modificada para que esses dados venham do WPSRead"""
    ...
    def calc_corner_point_latlon(center_lat, center_lon, e_we,
                                 e_ns, dx, dy,
                                 wpsproj, latlonproj, loc):

        ### Calculos da grade apartir dos valores obtidos do WPSRead ###
        centerOfGridX, centerOfGridY = wpsproj.transform_point(center_lon,
                                                               center_lat,
                                                               latlonproj)
        if loc == 'll':
            xGridPoints = centerOfGridX - dx*e_we/2.0
            yGridPoints = centerOfGridY - dy*e_ns/2.0
        elif loc == 'lr':
            xGridPoints = centerOfGridX + dx*e_we/2.0
            yGridPoints = centerOfGridY - dy*e_ns/2.0
        elif loc == 'ul':
            ...
            xGridPoints = centerOfGridX + dx*e_we/2.0
            yGridPoints = centerOfGridY + dy*e_ns/2.0
        corner_lon, corner_lat = latlonproj.transform_point(xGridPoints,
                                                            yGridPoints,
                                                            wpsproj)

        return corner_lon, corner_lat

    def calc_center_point_latlon(corner_lat_parent, corner_lon_parent,
                                 dx_parent, dy_parent,
                                 e_we, e_ns,
                                 dx, dy,
                                 i, j,
        ...
        centerOfGridX_child = corner_x_parent + dx_parent*i + dx*e_we/2.0
        centerOfGridY_child = corner_y_parent + dy_parent*j + dy*e_ns/2.0
        center_lon_child, center_lat_child = latlonproj.transform_point(centerOfGridX_child,
                                                                        centerOfGridY_child,
                                                                        wpsproj)
    ...
    def calc_wps_domain_info(wps_file):
        # TUDO ISSO DAQUI SERA MODIFICADO"""
        ndomain = get_wps_param_value(wps_file, 'max_dom', 1, 'int')
        proj_name = get_wps_param_value(wps_file, 'map_proj', 1, '')
        grid_ratios = get_wps_param_value(
            wps_file, 'parent_grid_ratio', ndomain, 'int')
        i_parent_start_array = get_wps_param_value(
            wps_file, 'i_parent_start', ndomain, 'int')
        j_parent_start_array = get_wps_param_value(
            wps_file, 'j_parent_start', ndomain, 'int')
        ...
        if ndomain > 1:
            for i in np.arange(1, ndomain):
                ...
                center_lon_full[i], center_lat_full[i] = calc_center_point_latlon(corner_lat_full[i-1, 0], 
